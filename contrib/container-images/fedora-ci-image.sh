...
build=$(buildah from quay.io/fedora/fedora:42)

# Ensure everything is up to date and install requirements
buildah run "${build}" -- /bin/bash -c "dnf update -y && dnf install -y git awk hostname nodejs nodejs20 tox which python3 python3-pip"
