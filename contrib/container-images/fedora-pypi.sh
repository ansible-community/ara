...
# Builds an ARA API server container image using the latest PyPi packages on Fedora 44.
...
buildah config --cmd "bash -c '/usr/local/bin/ara-manage migrate && python3 -m gunicorn --workers=4 --threads=4 --worker-class gthread --access-logfile - --bind 0.0.0.0:8000 ara.server.wsgi'" "${build}"
